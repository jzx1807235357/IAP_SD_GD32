 ...
 *
 * 参考官方 FMC 扇区擦除示例：
 * - GD32F4xx_Firmware_Library/Examples/FMC/Write_protection/main.c
 * - GD32F4xx_Firmware_Library/Examples/FMC/Program_erase/fmc_operation.c
 */
...
static flash_status_t flash_page_erase_internal(uint32_t page_address)
{
...
flash_status_t flash_page_erase(uint32_t page_address)
{
    ...
    status = flash_page_erase_internal(page_address);
    flash_end();
    return status;
}
